...
SCENARIO("RmsTestCases", "[rmsTestCases]")
{

    auto rmst = new RmsTest();

    GIVEN("Test gain")
    {
        WHEN("Configuring RMS")
        {
            rmst->rms->gain(DEG_TO_RAD(-35), DEG_TO_RAD(10), 0, 0);
    ...
    GIVEN("Computation of gains")
    {
        WHEN("Configuring RMS")
        {
            ...
            for (auto a : angles) {
                ...
                rmst->writeGains(ss.str(), DEG_TO_RAD(a.first), DEG_TO_RAD(a.second), 0, 0);
            }
            THEN("True, False, False")
            {
                REQUIRE(true);
