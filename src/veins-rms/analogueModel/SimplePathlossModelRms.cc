...
#include "SimplePathlossModelRms.h"
...
void SimplePathlossModelRms::filterSignal(Signal* signal)
{
    throw cRuntimeError("SimplePathlossModelRms::filterSignal(Signal* signal) must not be used as the model requires the AirFrame as well to perform computation");
}

void SimplePathlossModelRms::filterSignal(Signal* signal, AirFrame* frame)
{

    AirFrameRms* frameRms = check_and_cast<AirFrameRms*>(frame);
    ...
    frameRms->setTotalDistance(frameRms->getTotalDistance() + distance);

    // we do not apply path loss at the metasurface, only at the final receiver
    if (receiver->isReflectiveMetaSurface())
        return;

    sqrDistance = std::pow(frameRms->getTotalDistance(), 2);
