...
class PhyLayerRms : public BasePhyLayer {
public:
    void initialize(int stage) override;

    bool isReflectiveMetaSurface() const;

    const Coord& getRms_v2() const;
    const Coord& getRms_v1() const;
    const Coord& getRms_vn() const;
    ...
    /**
     * Request the reconfiguration of the RMS for a tx/rx pair of vehicles
...
protected:
    ...
    // determine whether this is an RMS or an UE
    bool isRMS;
    // vector indicating the orientation of the RMS
    // see the .ned file for a detailed description
    Coord rms_v2;
    Coord rms_v1;
    // vector indicating the direction the RMS is facing
    ...
    // map from vehicle id to their coordinates, to enable reconfiguration of the RMS
    ...
    /**
     * @brief Creates and initializes a RmsPathLoss with the
     * passed parameter values.
     */
    std::unique_ptr<AnalogueModel> initializeRmsPathLoss(ParameterMap& params);

    /**
     * @brief Creates and initializes a SimplePathLoss with the
     * passed parameter values.
     */
    std::unique_ptr<AnalogueModel> initializeSimplePathlossModelRms(ParameterMap& params);
    ...
    /**
     * @brief Initializes a new DeciderRms from the passed parameter map.
     */
    virtual std::unique_ptr<Decider> initializeDeciderRms(ParameterMap& params);

    /**
     * Create a protocol-specific AirFrame
     * Overloaded to create a specialize AirFrameRms.
     */
